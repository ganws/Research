...
function [fig_subplt] = plotTimeSeries(Dstruct, varargin)
    ...
    fig_subplt = figure;
    for i = 1:nVar
        subplot(3, round(nVar/3), i)
        for j = 1:nClass
            % x-axis as time axis
            if timeAxisPlt == true 
                dt = t_indx(2,1) - t_indx(1,1);
                tAxis = dt*frame;
                hp = plot(tAxis, D(indx_var{i}, indx_class{j}),classLine{j}) ;

                xlabel('Time [s]');
            % x-axis as sampling point
            else 
                hp = plot(D(indx_var{i}, indx_class{j}),classLine{j}) ;
